...
source("./1_reoa_matrices_generation_mean_sample_weight.R")
...
## Stable pairs
REOs <- readr::read_tsv("./results/reoa/4_med_sample_w/stable_pairs_0_verbose.dat", col_names = F)
...
## Upregulated
REOs_up <- readr::read_tsv("./results/reoa/4_med_sample_w/up_regulated_0.dat", col_names = F)
colnames(REOs_up) <- c("Protein","FDR")
REOs_up$Protein <- REOs_up$Protein + 1
REOs_up$Protein <- as.character(REOs_up$Protein)

## Downregulated
REOs_down <- readr::read_tsv("./results/reoa/4_med_sample_w/down_regulated_0.dat", col_names = F)
...
REOA_bars <- ggplot(iDEA_to_plot, aes(y=MED, x=combined))+
  ...
  guides(size = "none")
REOA_bars
ggsave(REOA_bars, filename = "./plots/4_med_sample_w/REOA_barmap.pdf")
...
REOA_bars2
ggsave(REOA_bars2, filename = "./plots/4_med_sample_w/REOA_barmap_top_dysregulated.pdf")
...
for(k in 1:length(up_list)){
  ...
  openxlsx::write.xlsx(x = main_functions, file = paste0("./results/functional_annotation/4_med_sample_w/",names(up_list)[k],".xlsx"))
  # # do the plot
  ...
  ggsave(plot  =go1_collapsed, filename = paste0("./plots/4_med_sample_w/",names(up_list)[k],".pdf"),
  ...
  openxlsx::write.xlsx(x = res_original, 
                       file = paste0("./results/functional_annotation/4_med_sample_w/raw_",names(up_list)[k],".xlsx"))
}
...
for(k in 1:length(down_list)){
  ...
  if (nrow(res) == 0){
    ...
    # extract the main functions and save the result as file
    main_functions <- res@result[res@result$ID %in% collapes_res$mainPaths,]
    openxlsx::write.xlsx(x = main_functions, file = paste0("./results/functional_annotation/4_med_sample_w/",names(down_list)[k],".xlsx"))
    # # do the plot
    go1_collapsed <- ggplot(top_n(main_functions,
                                  n = nrow(main_functions), wt = -p.adjust),
                            aes(x=Count, y=reorder(Description,Count), fill=p.adjust))+
    ...
    ggsave(plot  =go1_collapsed, filename = paste0("./plots/4_med_sample_w/",names(down_list)[k],".pdf"),
           width = 16, height = 4.5, units = "cm")}
  ...
  res_original <- res_original %>% 
    arrange(collapsed_to,p.adjust)
  openxlsx::write.xlsx(x = res_original, 
                       file = paste0("./results/functional_annotation/4_med_sample_w/raw_",names(down_list)[k],".xlsx"))
...
vl_plot
ggsave(vl_plot, filename = "./plots/4_med_sample_w/REOA_vlplot.pdf")
...
pdf("./plots/4_med_sample_w/REOA_hmap.pdf", width = 15, height = 10)
