...
source("./1_reoa_matrices_generation_e72_c2_e32.R")
...
## Stable pairs
REOs <- readr::read_tsv("./results/reoa/5_three_samples/stable_pairs_0_verbose.dat", col_names = F)
...
## Upregulated
REOs_up0 <- readr::read_tsv("./results/reoa/5_three_samples/up_regulated_0.dat", col_names = F)
REOs_up1 <- readr::read_tsv("./results/reoa/5_three_samples/up_regulated_1.dat", col_names = F)
REOs_up2 <- readr::read_tsv("./results/reoa/5_three_samples/up_regulated_2.dat", col_names = F)
REOs_up <- rbind(REOs_up0,
                 REOs_up1,
                 REOs_up2)
...
REOs_up <- REOs_up %>% 
  ...
  mutate(FDR = mean(FDR)) %>% 
  ungroup() %>% 
  subset(select = -c(apparances)) %>% 
  distinct()

## Downregulated
REOs_down0 <- readr::read_tsv("./results/reoa/5_three_samples/down_regulated_0.dat", col_names = F)
REOs_down1 <- readr::read_tsv("./results/reoa/5_three_samples/down_regulated_1.dat", col_names = F)
REOs_down2 <- readr::read_tsv("./results/reoa/5_three_samples/down_regulated_2.dat", col_names = F)
REOs_down <- rbind(REOs_down0,
                 REOs_down1,
                 REOs_down2)
...
individualized_problem_illness <- expression_matrix_problem_illness %>% 
  tidyr::pivot_longer(cols = all_of(samples), 
                      names_to = "sample", values_to = "abundance")
individualized_problem_illness$sample <- "C110"
...
REOA_bars <- ggplot(iDEA_to_plot, aes(y=MED, x=combined))+
  ...
  guides(size = "none")
REOA_bars
ggsave(REOA_bars, filename = "./plots/5_three_samples/REOA_barmap.pdf")
...
REOA_bars2
ggsave(REOA_bars2, filename = "./plots/5_three_samples/REOA_barmap_top_dysregulated.pdf")
...
for(k in 1:length(up_list)){
  ...
  openxlsx::write.xlsx(x = main_functions, file = paste0("./results/functional_annotation/5_three_samples/",names(up_list)[k],".xlsx"))
  # # do the plot
  ...
  ggsave(plot  =go1_collapsed, filename = paste0("./plots/5_three_samples/",names(up_list)[k],".pdf"),
  ...
  openxlsx::write.xlsx(x = res_original, 
                       file = paste0("./results/functional_annotation/5_three_samples/raw_",names(up_list)[k],".xlsx"))
}
...
for(k in 1:length(down_list)){
  ...
  if (nrow(res) == 0){
    ...
    # extract the main functions and save the result as file
    main_functions <- res@result[res@result$ID %in% collapes_res$mainPaths,]
    openxlsx::write.xlsx(x = main_functions, file = paste0("./results/functional_annotation/5_three_samples/",names(down_list)[k],".xlsx"))
    # # do the plot
    go1_collapsed <- ggplot(top_n(main_functions,
                                  n = nrow(main_functions), wt = -p.adjust),
                            aes(x=Count, y=reorder(Description,Count), fill=p.adjust))+
    ...
    ggsave(plot  =go1_collapsed, filename = paste0("./plots/5_three_samples/",names(down_list)[k],".pdf"),
           width = 16, height = 4.5, units = "cm")}
  ...
  res_original <- res_original %>% 
    arrange(collapsed_to,p.adjust)
  openxlsx::write.xlsx(x = res_original, 
                       file = paste0("./results/functional_annotation/5_three_samples/raw_",names(down_list)[k],".xlsx"))
  
}

### Barplot on the up/down genes
# up_to_check <- BP_up_table
...
# vl_plot
# ggsave(vl_plot, filename = "./plots/5_three_samples/REOA_vlplot.pdf")
...
patients_grouping <- as.data.frame(tibble::tibble(
  patient = c("A10","A102","A103","A105","A107","A108","A112","A113","A114","A115",
              ...
              "C100","C101","C117","C20" ,"C30","C33","C37","C41","C45","C48","C54","C57","C58","C60","C62","C63",
              "C64" ,"C65","C71","E109","E111","E118","E29","E39","E55","E61",
              "E68","E76","E82","E89","E90","E95","C110","E72","C2","C32"),
  grouping = c(rep("Cohort",times = 52),
               "C110","E72","C2","C32")))
...
## Proteins to plot
prots_to_plot <- iDEA %>% 
  filter(condition %in% c("Downregulated","Upregulated"))
prots_to_plot <- unique(prots_to_plot$Gene.names)
...
prots_annot <- iDEA %>% 
  filter(condition %in% c("Downregulated","Upregulated")) %>% 
  subset(select = c(Gene.names,condition)) %>% 
  distinct() %>% 
...
### Create the splits
split_row <- factor(group_annot$Groups, levels =c("C110","E72","C2","C32","Cohort"))
...
pdf("./plots/5_three_samples/REOA_hmap.pdf", width = 15, height = 10)
