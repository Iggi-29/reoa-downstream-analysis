...
source("./1_reoa_matrices_generation_most_similar_to_all.R")
...
## Stable pairs
REOs <- readr::read_tsv("./results/reoa/2_most_similar_to_all/stable_pairs_0_verbose.dat", col_names = F)
...
## Upregulated
REOs_up <- readr::read_tsv("./results/reoa/2_most_similar_to_all/up_regulated_0.dat", col_names = F)
colnames(REOs_up) <- c("Protein","FDR")
REOs_up$Protein <- REOs_up$Protein + 1
REOs_up$Protein <- as.character(REOs_up$Protein)

## Downregulated
REOs_down <- readr::read_tsv("./results/reoa/2_most_similar_to_all/down_regulated_0.dat", col_names = F)
...
REOA_bars <- ggplot(iDEA_to_plot, aes(y=MED, x=combined))+
  ...
  guides(size = "none")
REOA_bars
ggsave(REOA_bars, filename = "./plots/2_most_similar_to_all/REOA_barmap.pdf")
...
REOA_bars2
ggsave(REOA_bars2, filename = "./plots/2_most_similar_to_all/REOA_barmap_top_dysregulated.pdf")
...
for(k in 1:length(up_list)){
  ...
  openxlsx::write.xlsx(x = main_functions, file = paste0("./results/functional_annotation/2_most_similar_to_all/",names(up_list)[k],".xlsx"))
  # # do the plot
  ...
  ggsave(plot  =go1_collapsed, filename = paste0("./plots/2_most_similar_to_all/",names(up_list)[k],".pdf"),
  ...
  openxlsx::write.xlsx(x = res_original, 
                       file = paste0("./results/functional_annotation/2_most_similar_to_all/raw_",names(up_list)[k],".xlsx"))
}
...
for(k in 1:length(down_list)){
  ...
  if (nrow(res) == 0){
    ...
    # extract the main functions and save the result as file
    main_functions <- res@result[res@result$ID %in% collapes_res$mainPaths,]
    openxlsx::write.xlsx(x = main_functions, file = paste0("./results/functional_annotation/2_most_similar_to_all/",names(down_list)[k],".xlsx"))
    # # do the plot
    go1_collapsed <- ggplot(top_n(main_functions,
                                  n = nrow(main_functions), wt = -p.adjust),
                            aes(x=Count, y=reorder(Description,Count), fill=p.adjust))+
    ...
    ggsave(plot  =go1_collapsed, filename = paste0("./plots/2_most_similar_to_all/",names(down_list)[k],".pdf"),
           width = 16, height = 4.5, units = "cm")}
  ...
  res_original <- res_original %>% 
    arrange(collapsed_to,p.adjust)
  openxlsx::write.xlsx(x = res_original, 
                       file = paste0("./results/functional_annotation/2_most_similar_to_all/raw_",names(down_list)[k],".xlsx"))
...
vl_plot
ggsave(vl_plot, filename = "./plots/2_most_similar_to_all/REOA_vlplot.pdf")
...
patients_grouping <- as.data.frame(tibble::tibble(
  patient = c("A10","A102","A103","A105","A107","A108","A112","A113","A114","A115",
              ...
              "C64" ,"C65","C71","E109","E111","E118","E29","E39","E72","E61",
              "E68","E76","E82","E89","E90","E95","C110","E55"),
  grouping = c(rep("Cohort",times = 54),
               "C110","E55")))
...
### Create the splits
split_row <- factor(group_annot$Groups, levels =c("C110","E55","Cohort"))
...
pdf("./plots/2_most_similar_to_all/REOA_hmap.pdf", width = 15, height = 10)
